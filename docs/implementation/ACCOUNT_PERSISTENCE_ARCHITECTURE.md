...
No static session-signing or cookie-encryption secret is required by this design. Session tokens are high-entropy bearer values whose hashes are the only values persisted server-side. Production and Preview HTTPS requests use `Secure`, `HttpOnly`, `SameSite=Lax`, host-only `__Host-` cookies with `Path=/` and no `Domain` attribute.
...
4. `npm run db:migrate:test` applies every ordered migration twice to a clean isolated in-memory PostgreSQL-compatible test database.
