...
describe('Result Workspace composition', () => {
  ...
  it('opens with a simplified repository-specific entry and routes the primary action to Repository Intelligence review', async () => {
    ...
    expect(within(futureField).getAllByText(/Current evidence|Intervention|Decision \+ dependencies|Future outcome/)).toHaveLength(4);
    expect(futureField.querySelectorAll('[data-future-node="bundle"]').length).toBeGreaterThan(0);
    ...
    const fieldCandidate = within(futureField).getAllByRole('button').find(button => button.getAttribute('aria-label')?.includes('Activate to choose as primary'))!;
    fireEvent.mouseEnter(fieldCandidate);
    expect(await screen.findByTestId('future-context-inspector')).toBeInTheDocument();
    fireEvent.mouseLeave(fieldCandidate);
    await waitFor(() => expect(screen.queryByTestId('future-context-inspector')).not.toBeInTheDocument());
    fireEvent.click(fieldCandidate);
    expect(await screen.findByTestId('future-context-inspector')).toBeInTheDocument();
    expect(screen.getByTestId('future-selected-path-summary')).toHaveTextContent(/Primary.*Supporting.*Requires/);
    expect(within(impactMode).getByRole('button', { name: 'Current repository' })).toHaveAttribute('aria-pressed', 'true');
    ...
    fireEvent.click(within(screen.getByTestId('future-context-inspector')).getByRole('button', { name: 'Clear' }));
    await waitFor(() => expect(screen.queryByTestId('future-context-inspector')).not.toBeInTheDocument());
