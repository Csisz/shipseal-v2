...
export default function SavedScan() {
  ...
  return <div className="min-h-screen bg-background"><Nav />{!report ? <main className="container pt-24 md:pt-28">{error ? <div className="rounded-2xl border border-destructive/40 bg-destructive/10 p-6"><h1 className="font-display text-2xl font-semibold">Saved scan cannot be reopened</h1><p className="mt-2 text-sm text-muted-foreground">{error}</p><Button className="mt-4" onClick={() => navigate(`/projects/${projectId}`)}>Back to project</Button></div> : <p className="text-muted-foreground">Validating saved scan…</p>}</main> : <main className="pt-16 md:pt-20"><div className="container pt-4"><div className="rounded-xl border border-primary/25 bg-primary/5 px-4 py-3 text-sm"><span className="font-medium">Saved scan</span><span className="ml-2 text-muted-foreground">Opened without rescanning, provider execution, or GitHub mutation. {scan?.intelligenceMode} mode.</span></div></div><Suspense fallback={<div className="container py-24 text-muted-foreground">Loading saved result…</div>}><ResultDashboard report={report} history={[]} onReset={() => navigate(`/projects/${projectId}`)} onClearHistory={() => undefined} /></Suspense></main>}</div>;
}
